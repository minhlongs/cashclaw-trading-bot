...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const rawExchange = req.nextUrl.searchParams.get('exchange') ?? undefined;
  const rawSymbol = req.nextUrl.searchParams.has('symbol')
    ? req.nextUrl.searchParams.get('symbol')
    : undefined;

  const parseResult = TickerQuerySchema.safeParse({
