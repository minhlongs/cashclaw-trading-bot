...
const ZBAND = 5e-6;
const END_MS = new Date('2025-09-19T00:00:00Z').getTime();
const DAYS = 730;
...
function bci(s: number[]): { lo: number; hi: number } {
  if (s.length < 3) return { lo: 0, hi: 0 };
  const m: number[] = [];
  for (let r = 0; r < NRESAMP; r++) { let sm = 0; for (let i = 0; i < s.length; i++) sm += s[Math.floor(Math.random() * s.length)]; m.push(sm / s.length); }
  m.sort((a, b) => a - b);
  return { lo: m[Math.floor(NRESAMP * 0.025)], hi: m[Math.floor(NRESAMP * 0.975)] };
}

function metrics(ts: Trade[]): Metrics {
  if (!ts.length) return NM;
  const pn = ts.map(t => t.pnl);
  const pnl = pn.reduce((a, b) => a + b, 0);
  ...
  const mu = pnl / ts.length;
  const sd = Math.sqrt(pn.reduce((s, v) => s + (v - mu) ** 2, 0) / pn.length);
  const ah = (ts.reduce((s, t) => s + t.bars, 0) / ts.length) * 8;
  const tpy = ah > 0 ? (365.25 * 24) / ah : 0;
  const sh = sd === 0 || tpy === 0 ? 0 : (mu / sd) * Math.sqrt(tpy);
  const ci = bci(pn);
...
function report(fp: FP[], res: SR[]): string {
  ...
  md.push(`| Periods | ${fp.length} | Mean | ${fmt(mu)} | Std | ${fmt(sd)} |`);
  md.push(`| Min | ${fmt(Math.min(...rates))} | Max | ${fmt(Math.max(...rates))} |`);
  md.push(`| Positive | ${pos} (${(pos / rates.length * 100).toFixed(1)}%) | Negative | ${neg} (${(neg / rates.length * 100).toFixed(1)}%) |\n');
...
async function main() {
  ...
  console.log('Symbol: SOLUSDT | Days: 730 | End: 2025-09-19\n');
  console.log('Fetching funding...');
  const fp = await fetchFunding('SOLUSDT');
  console.log(`  ${fp.length} periods`);
  console.log('Fetching 8h candles...');
  const candles = await fetchCandles('SOLUSDT');
