...
const TEMPLATES: HypothesisTemplate[] = [
  ...
  { name: 'mean-revert', description: 'Mean-reversion strategy', indicatorPreset: [{ indicator: 'bollinger', lookback: 20 }, { indicator: 'rsi', lookback: 14 }], regimePreset: [RegimeLabel.RANGE], barrierPreset: { takeProfitPct: 0.01, stopLossPct: 0.01, maxHoldingMs: 6 * 3600_000 }, combinePreset: 'max_confidence' },
  { name: 'momentum-burst', description: 'Momentum-based entry', indicatorPreset: [{ indicator: 'momentum', lookback: 10 }, { indicator: 'volume_zscore', lookback: 20 }], regimePreset: [RegimeLabel.TREND_UP, RegimeLabel.HIGH_VOLATILITY], barrierPreset: { takeProfitPct: 0.03, stopLossPct: 0.015, maxHoldingMs: 12 * 3600_000 }, combinePreset: 'voting' },
  { name: 'vol-surf', description: 'Volatility surface', indicatorPreset: [{ indicator: 'atr', lookback: 14 }, { indicator: 'realized_volatility', lookback: 30 }], regimePreset: [RegimeLabel.HIGH_VOLATILITY], barrierPreset: { takeProfitPct: 0.04, stopLossPct: 0.02, maxHoldingMs: 8 * 3600_000 }, combinePreset: 'weighted_sum' },
  { name: 'dual-sma', description: 'Dual SMA crossover', indicatorPreset: [{ indicator: 'sma', lookback: 20 }, { indicator: 'sma', lookback: 50 }], regimePreset: [RegimeLabel.TREND_UP, RegimeLabel.TREND_DOWN], barrierPreset: { takeProfitPct: 0.025, stopLossPct: 0.012, maxHoldingMs: 48 * 3600_000 }, combinePreset: 'voting' },
...
describe('HypothesisGenerator — generateRegimeSpecificHypothesis', () => {
  const regimes: RegimeLabel[] = [RegimeLabel.TREND_UP, RegimeLabel.TREND_DOWN, RegimeLabel.RANGE, RegimeLabel.HIGH_VOLATILITY, RegimeLabel.LOW_VOLATILITY, RegimeLabel.SHOCK];
  ...
  it('uses regime-appropriate barrier config', () => {
    const gen = new HypothesisGenerator();
    const trendUp = gen.generateRegimeSpecificHypothesis(RegimeLabel.TREND_UP);
    expect(trendUp.barrierConfig.takeProfitPct).toBe(0.03);
    const range = gen.generateRegimeSpecificHypothesis(RegimeLabel.RANGE);
