// QuantLib strategy implementations (grid, mean_reversion).
// Pure quantitative functions preserving 100% test coverage floor.
 ...
 */
export function grid(ctx: QuantLibContext, params?: Record<string, number>): QuantResult {
  if (ctx.lastPrice <= 0) {
 ...
 */
export function meanReversion(ctx: QuantLibContext, params?: Record<string, number>): QuantResult {
  if (ctx.lastPrice <= 0) {
