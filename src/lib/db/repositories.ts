...
export async function upsertSettings(db: D1Database, row: SettingsRow): Promise<void> {
  await db.prepare(
    `INSERT INTO settings (id, user_id, exchange_creds_json, risk_limits_json, notification_json, killswitch_daily_json, killswitch_enabled, killswitch_reason, killswitch_triggered_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(id) DO UPDATE SET
       ...
       killswitch_daily_json = excluded.killswitch_daily_json,
    ...
    row.killswitch_daily_json, row.killswitch_enabled, row.killswitch_reason, row.killswitch_triggered_at, row.updated_at
  ).run();
}
