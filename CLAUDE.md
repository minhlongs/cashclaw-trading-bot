# Project Context

## Current Status (2026-09-11)
- **End-to-End Volatility-DCA Bot Integration** — Complete. Full cross-stack integration of QuantLib `volatilityDca` strategy module into CashClaw bot execution engine, Cloudflare D1 persistence, API endpoints, and UI wizard. Deployed live to Cloudflare Workers (commit `b4cb0706`). 3,881/3,881 tests passing across 298 test files with 100% statement/branch/function/line coverage on `src/tree/bot/strategies/volatility-dca.ts`, D1 migration `0012` applied, bilingual en/vi localization.
- **Direct Web-API REST, QuantLib Volatility-DCA, & Strategy Visualizer UI** — Complete. Edge-native WebCrypto HMAC-SHA256 signer (`crypto.subtle`) + stateless Binance REST client preserving ADR-001 paper-only invariant with 100% test coverage; deterministic Volatility-Adjusted DCA (`volatilityDca`) strategy module in `src/tree/quantlib/` maintaining strict 100.00% statement/branch/function/line coverage floor across 80 tests; responsive `StrategyVisualizerCard` dashboard component with semantic CSS tokens (zero inline styles) and bilingual localization. 3,865/3,865 tests passing across 297 test files.
- **Sweep Artifact Documentation & Consistency Pin** — Complete (commit `de5d809`). 16-test invariant suite in `src/forest/alpha/composition-eval/sweep-artifact.test.ts` pins schema, universe, scan zero-pair result, correlation ordering, 16 walk-forward windows, survival gate KILLED with 4 economic fails / 4 structural passes, and RESEARCH→KILLED promotion path. Companion report `docs/multi-pair-composition-sweep-report.md` added. 3803/3803 tests passing.
- **QuantLib Branch Coverage Gap Closure & Cloudflare Workers Go-Live** — Complete (commits `1eefb2f`, `2552754`, `6839d17`). Parameter-aware QuantLib strategy functions (`grid` with multi-level depth & `meanReversion`), 100% test coverage floor across all metrics in `src/tree/quantlib/**` (54 tests), multi-pair composition walk-forward sweep report committed, ADR-001 CCXT Workers feasibility resolved (firm NO-GO, v1 locked to paper mode), and production live on Cloudflare Workers edge runtime (SHA `6839d174`). 3787/3787 tests passing.
- **Alpha Research OS Backlog Closure** — Complete. Multi-pair universe scan causal wiring (`scanMultiPairUniverse`), rolling correlation diagnostics (`computeRollingCorrelationSeries`), walk-forward composition evaluation (`runCompositionWalkForward`, `toWalkForwardShim`), and automated pipeline survival-gate / promotion-state lifecycle integration (`stepGenerateReport`). 3751/3751 tests passing.
- **Direct-D1 Read Store & Anti-IDOR Authorization Hardening** — Complete (commits `aff8f46`, `71f9a92`). Direct-D1 read architecture for `BotManager` (`getAllBots`, `getRunningBots`, `getBot`) establishing Cloudflare D1 as single source of truth across worker isolate recycles; in-memory TTL cache (`BOT_CACHE_TTL_MS = 30_000`) for hot-path scheduler ticks; multi-tenant user scoping and anti-IDOR authorization enforcement across all queries, cache entries, and bot lifecycle operations (`getCachedBotOrThrow`). 3734/3734 tests passing.
- **UI/UX Token Migration** — Complete (commits `ba6cd64`, `cdeae09`). Migrated 41 files from inline styles to semantic CSS tokens and utility classes.
- **CashClaw × Vibe-Trading Alpha Zoo Evaluator + IC/IR Falsification Bridge Phase 3** — Complete. Zoo alpha evaluator + factor IC/IR falsification bridge: causal operator parser + evaluator kernels (`operator-{ast,tokenizer,parser,kernels,cross,pair,evaluator}.ts`, 17-op vocabulary, append-invariant, no lookahead), factor panel + IC/IR analysis (`factors/{panel,ic-analysis,ic-metrics,ic-quantile}.ts` — forward returns, bootstrap CI, Spearman/Pearson, quantile spread, turnover), forest-layer `runZooFalsification` bridge (import → parse → evaluate → analyzeIc → bootstrap CI + permutation + walk-forward shim; three-terminal verdicts; fail-closed Σ≡N; deterministic FNV-1a32 seeds). Seed run `scripts/zoo-phase3-falsify.ts` executes the committed 12-entry phase-2 manifest on the cached 3-symbol Binance 1d panel (730 bars) with all zoo market tags mapped to the crypto universe (documented adaptation, Phase 2 D4 precedent) → committed artifact `plans/reports/zoo-phase3-falsification-report.json` (0 alive / 2 falsified / 2 not-evaluable / 8 import-skipped, Σ≡12) + `docs/zoo-seed-falsification-report.md` with external-validity caveat (equity-designed alphas on 3-asset crypto panel = pipeline demonstration, not tradable conclusion). Consistency test pins the committed artifact byte-for-byte. All pure — no I/O in tree, no eval/exec, WebCrypto only. 158 new tests (3152 total), coverage 89.64% ≥ 89.09% baseline.
- **TradingAgents Deliberation Engine Adapter** — Complete (merged via PR #12). Multi-agent LLM deliberation adapter, PAPER/BACKTEST advisory only — no order surface. Tree contracts (`DebateState`, `ModelProvenance`, `RiskScenarioSet`, `DecisionContract`, security gate) at commit `fcb995b`; forest orchestration (`ModelRouter` FAST/REASONING/LOCAL tiered routing, `ProviderRegistry` 9-provider allowlist, debate orchestrator, `debateToHypothesis` → CashClaw compile, risk/portfolio advisors over `buildPortfolio`, hash-chained `DecisionLogWriter`, `runDeliberation` seam with Σ≡N, calibration + debate-quality metrics, checkpoint adapter). Deterministic demo script + committed byte-identical artifacts (JSON + bilingual markdown) guarded by consistency test; D11 label marks fixture provider as TEST seam. 34 forest tests (3194 total), quality gate exit 0, coverage 88.59% ≥ thresholds 82/85/85/82.
- **Prior: CashClaw × Vibe-Trading Alpha Zoo Adapter Phase 2** — Complete. Pure tree-layer ingestion of Vibe-Trading zoo metadata into validated `ResearchHypothesis` candidates: compiler split (`compile-stages.ts`, dead placeholder seed removed), zoo metadata Zod schemas (snake_case verbatim), operator vocabulary + fail-closed formula normalizer, fail-closed `AlphaImportReport` (7 buckets, Σ ≡ N enforced), deterministic `AlphaZooAdapter` with canonical-payload dedup, 12-entry seed manifest transcribed verbatim from vibe-trading @ `5cd08ee1bd5c` + golden audit test pinning per-entry outcomes (4 adapted / 7 unsupported / 1 validation-error). All pure — no I/O, no eval/exec, WebCrypto only. 82 new tests (2994 total).
- **Prior: CashClaw × Vibe-Trading Research Contracts Phase 1** — Complete. Tree-layer purity research contracts: `ResearchHypothesis` + `ResearchGoal` Zod schemas with mechanism gate, `AlphaProvenance` (formula hash + normalized representation + validation), `EvidenceObject`/`ResearchLineage` append-only lineage graph, deterministic `AlphaCompiler` pipeline (6 stages: Zod+mechanism gate → causal validation via `declareFeature(causal:true)` → feature validation → data/universe window coverage → cost validation via `resolveStressConfig` → emit `ExperimentSpec` with SHA-256 specId). All pure — no I/O, no eval/exec, WebCrypto only. 109 new tests (2912 total), coverage 88.86% ≥ 88.22% baseline.
- **Alpha Research OS** — All 6 phases complete. Phase 6 composition shipped 2026-08-25 (PR #6, commit `985c9f1`): alpha composition scoring, 9-overlay portfolio engine, EXTREME cost mode, forest evaluation suite with leakage-isolation
- **Prior phases:** 1 (evaluator engine), 2 (research queue + multiple-testing), 3 (microstructure data), 4 (cross-sectional engine, `b7d5454`), 5 (relative-value research, `b3f51fc`)
- **System state:** Paper/backtest only. No live capital. 3803/3803 tests passing, quality gate green.
- **Next work:** All known backlog items closed. Production live on Cloudflare Workers (`https://cashclaw-trading-bot.agencyos-openclaw.workers.dev`).

## Safety Rules
1. PAPER/BACKTEST ONLY — no real orders, no live trading
2. Never use future data in features, labels, regime detection, or execution
3. Every backtest must include fees and configurable slippage
4. Preserve existing tests and functionality
5. Small, reversible commits

## Code Standards
- TypeScript, 0 `:any` types
- File naming: kebab-case, max 200 lines
- No `console.log` in production
- Zod validation on API inputs
- YAGNI, KISS, DRY
- `npm test` must pass before commit
- Conventional commit messages

## Architecture Quick Reference
```
src/
├── tree/          # Data models, exchange adapters, provider chain
├── land/          # Exchange orchestration, bot control
├── forest/        # Strategies, backtest, dashboard, regime engine
├── quantlib/      # Quantitative functions (placeholder)
```

## Key Files
- `src/tree/exchange/provider/provider.ts` — ProviderChain + interface definitions
- `src/tree/exchange/provider/paper-provider-adapter.ts` — PaperExchangeProvider → TickerProvider/OrderProvider bridge
- `src/land/exchange-orchestration/index.ts` — ExchangeOrchestrator (wired to ProviderChain)
- `src/forest/backtest/` — Hypothesis sweep scripts (archived)
- `docs/falsification-report.md` — Final falsification results
- `docs/development-roadmap.md` — Project phases and backlog
