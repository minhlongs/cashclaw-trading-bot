...
| **Direct Ticker Provider & Paper Exchange Market Data** (2026-09-12) | Unified direct REST engine into CashClaw core trading hierarchy: bidirectional `symbol-normalizer.ts` (canonical `BASE/QUOTE` <-> Binance `BTCUSDT`, OKX `BTC-USDT`, Bybit `BTCUSDT`), robust fail-closed `ticker-normalizer.ts` (`normalizeTicker`, `validatePositiveFinite`), unified `DirectTickerProvider` implementing `TickerProvider` with `CircuitBreaker` FSM and ping health check, and live market data pricing integration in `PaperExchange` with simulated fallback. Pruned `PaperExchangeProvider` below 200 LOC (< 190 LOC). Strictly preserved ADR-001 paper-only invariant (zero live order surface). 4,012/4,012 tests passing across 315 test files (+44 tests), 100.00% statement/branch/function/line coverage maintained on `src/tree/exchange/direct/**`, all 14 files ≤ 200 LOC, 0 `:any` types. | commit pending |
| **OKX & Bybit Direct REST Clients** (2026-09-12) | Edge-native WebCrypto expansion of `src/tree/exchange/direct/` with OKX (Base64 HMAC-SHA256, passphrase, simulated-trading header) and Bybit V5 (Hex HMAC-SHA256, recvWindow, sorted GET params) stateless REST clients. New `signHmacSha256Base64`, `buildOkxHeaders`/`buildOkxSignature`, `buildBybitHeaders`/`buildBybitSignature` in `webcrypto-signer.ts`. Both clients: SSRF protocol guard, public market data (`ping`, `getServerTime`, `fetchTicker`), signed request generator (`createSignedRequest`). ADR-001 paper-only invariant enforced (zero order execution). 92 new tests (3,968 total across 309 test files), 100.00% statement/branch/function/line coverage on `src/tree/exchange/direct/**`, all 10 files ≤ 200 LOC, 0 `:any` types, 0 Node.js imports. | commits in this session |
| **Direct-D1 Read Store with TTL Cache** (2026-09-01) | Direct-D1 read architecture for BotManager (`getAllBots`, `getRunningBots`, `getBot`) eliminating in-memory state desynchronization across ephemeral Cloudflare Workers isolates. Cloudflare D1 is the single authoritative source of truth. Hot-path in-memory TTL cache (`CachedBot` with `BOT_CACHE_TTL_MS = 30_000`) prevents query exhaustion during high-frequency scheduler ticks and dashboard polling. Transparent D1 reads replace prior manual pre-hydration phases in scheduler and worker. Fallback configuration synthesis (`defaultConfigFromRow`) for corrupt JSON records. Multi-tenant anti-IDOR user scoping enforced across all reads, cache lookups, and bot lifecycle operations (`getCachedBotOrThrow`). All quality gates green: 3734/3734 tests passing, typecheck 0, lint 0. | commits `aff8f46`, `71f9a92` |
| **CashClaw × Vibe-Trading Alpha Zoo Evaluator + IC/IR Falsification Bridge Phase 3** (2026-08-26) | Zoo alpha evaluator + factor IC/IR falsification bridge: causal operator parser + evaluator kernels (`operator-{ast,tokenizer,parser,kernels,cross,pair,evaluator}.ts` — 17-op vocabulary, append-invariant, no lookahead), factor panel + IC/IR analysis (`factors/{panel,ic-analysis,ic-metrics,ic-quantile}.ts` — forward-return series, bootstrap CI, Spearman/Pearson, quantile spread, turnover), forest-layer falsification bridge `runZooFalsification` (import → parse → evaluate → analyzeIc → bootstrap CI + permutation + walk-forward shim checks; three-terminal verdicts ALIVE/FALSIFIED/NOT_EVALUABLE; fail-closed Σ≡N via `assertNoSilentSkips`; deterministic FNV-1a32 seeds). Seed run `scripts/zoo-phase3-falsify.ts` executes the committed 12-entry phase-2 manifest on the cached 3-symbol Binance 1d panel (BTCUSDT/ETHUSDT/SOLUSDT, 730 bars) with ALL zoo market tags mapped to the crypto universe (documented adaptation, Phase 2 D4 precedent) → committed artifact `plans/reports/zoo-phase3-falsification-report.json` (0 alive / 2 falsified / 2 not-evaluable / 8 import-skipped, Σ≡12) + human-readable `docs/zoo-seed-falsification-report.md` with external-validity caveat (equity-designed alphas on 3-asset crypto panel = pipeline demonstration, not tradable conclusion). Consistency test re-runs the bridge on cached inputs and pins the committed artifact byte-for-byte. All pure — no I/O in tree, no eval/exec, WebCrypto only. 158 new tests (3152 total), coverage 89.64% ≥ 89.09% baseline | PR #11, commit `f579005` |
| **UI/UX Token Migration** (2026-08-31) | Replaced all inline `style={{...}}` in `src/app` + `src/components` with semantic CSS classes from `globals.css`/`tokens.css`. Design tokens: colors (profit/loss/warning/ai/error/accent), text hierarchy (primary/secondary/tertiary/muted), line-height (tight/normal/relaxed). New utility classes: text-*, badge-*, chart-svg, sidebar-logo, alert-level-error, error-state, review-error, save-message. Sidebar width hardcoded to 240px. i18n icon fields: emoji → semantic icon names (bot, chart, shield, briefcase, rocket, clipboard-list, plug, play, trending-up). LandingClient/CtaClient render static icon strings directly (fixes static-gen crash). 41 files migrated, 0 inline styles remain. Tests updated (container assertions). Quality: build 0, type-check 0, tests 3700/3700. | commits `ba6cd64`, `cdeae09` |
| **TradingAgents Deliberation Engine Adapter** (2026-08-27) | Multi-agent LLM deliberation adapter (paper/backtest advisory only — no order surface): tree-layer contracts (`DebateState`/`DebateRound` hash-chained state, `ModelProvenance`/`ModelProvenanceRecord` per-call accounting, `RiskScenarioSet` 3-view advisory, `DecisionContract` Zod fail-closed, security gate, data-extraction, research-synthesis); forest-layer orchestration (`ModelRouter` OmniRouter-compatible FAST/REASONING/LOCAL tiered routing with provider-reported latency + fallback, `ProviderRegistry` 9-provider allowlist, `runDebateOrchestrator` bull/bear debate + risk/portfolio advisors, `debateToHypothesis` → CashClaw compile, `generateRiskAdvisory`/`constrainConfigByAdvisory` advisory-only sizing, `advisePortfolio` via `buildPortfolio` 9 overlays, hash-chained `DecisionLogWriter`, `runDeliberation` top-level seam with Σ≡N `assertNoSilentSkips`, agent calibration + debate-quality metrics, checkpoint envelope adapter). Deterministic demo (`scripts/tradingagents-deliberation-demo.ts`) with committed byte-identical artifacts (JSON + bilingual markdown) guarded by consistency test; D11 honesty label marks `DeterministicFixtureProvider` as TEST seam. 34 forest tests (3194 total), quality gate exit 0, coverage 88.59% ≥ thresholds 82/85/85/82 | PR #12, commit `598bf25` |
| **CashClaw × Vibe-Trading Alpha Zoo Adapter Phase 2** (2026-08-26) | Pure tree-layer ingestion of Vibe-Trading zoo metadata into validated ResearchHypothesis candidates: compiler validation stages extracted to `compile-stages.ts` (dead placeholder seed removed from specId body), zoo metadata Zod schemas (snake_case `__alpha_meta__` verbatim), operator vocabulary + fail-closed formula normalizer (17-op alias table; placeholder/conditional/unknown-operator/non-causal classification), fail-closed `AlphaImportReport` taxonomy (7 buckets, Σ buckets ≡ N enforced by `assertNoSilentSkips`), deterministic `AlphaZooAdapter` (canonical-payload dedup via `computeFormulaHash`; hypothesis assembly reusing mechanism gate + `validateProvenance` + `AlphaCompiler.compile`), representative 12-entry seed manifest transcribed verbatim from vibe-trading @ `5cd08ee1bd5c` with golden audit test pinning observed per-entry outcomes (4 adapted / 7 unsupported / 1 validation-error; vsump5+vsump10 both register on distinct canonical payloads). All pure — no I/O, no eval/exec, WebCrypto only. 82 new tests (2994 total), coverage 89.09% ≥ 88.86% baseline | PR #10, commit `5be2bf0` |
| **CashClaw × Vibe-Trading Research Contracts Phase 1** (2026-08-26) | Tree-layer purity research contracts: `ResearchHypothesis` + `ResearchGoal` Zod schemas with mechanism gate (non-causal rejection), `AlphaProvenance` (formula hash + normalized representation + validation), `EvidenceObject`/`ResearchLineage` append-only lineage graph, deterministic `AlphaCompiler` pipeline (6 stages: Zod+mechanism gate → causal validation via `declareFeature(causal:true)` → feature validation with duplicates/lookbacks/allowlist → data/universe window coverage → cost validation via `resolveStressConfig` → emit `ExperimentSpec` with SHA-256 specId from canonical JSON excl compiledAt). All pure — no I/O, no eval/exec, WebCrypto only. 109 new tests (2912 total), coverage 88.86% ≥ 88.22% | PR #9, commit `a20d234` |
| **Alpha Research OS Phase 2** (2026-08-23) | Research queue with 7-state lifecycle (`PROPOSED→VALIDATING→RUNNING→EVALUATED→SURVIVED/FALSIFIED→ARCHIVED`, duplicate prevention via configHash, fail-closed `validateJobSpec`) in `src/tree/alpha/queue/`; multiple-testing defense (bootstrap CI, permutation/random-entry baseline, counters, PBO proxy, parameter sensitivity, walk-forward + cross-asset consistency, `evaluateSurvival` ANY-fail→falsified) in `src/forest/alpha/multiple-testing/`; append-only D1 store `migrations/0010_research_queue.sql` + `queue-d1-store.ts`. 184 new tests (2307 total), coverage 86.98% | PR #2, commit `db388f3` |
| **Alpha Research OS Phase 3** (2026-08-24) | Microstructure data infrastructure: fail-closed REST polling ingestion (Binance `depth?limit=20` + `aggTrades`) in `src/forest/alpha/microstructure/`; parse/quality/feature-computer domain in `src/tree/alpha/microstructure/` (9 causal feature contracts with publication-lag `asOf` gate); append-only D1 store `migrations/0011_microstructure_data.sql` (4 tables) + `micro-d1-store.ts`; worker cron wiring gated on `MICRO_INGEST_ENABLED` (default OFF). 93 new tests (2412 total), coverage thresholds held 82/85/85/82 | PR #3, commit `d4405c1` |
| **Alpha Research OS Phase 4** (2026-08-24) | Cross-sectional engine: causal portfolio simulator `runCrossSectionalSim` in `src/tree/alpha/cross-sectional/` (weights decided at snapshot t earn return t→t+1; one-sided turnover ½·Σ\|Δw\|; fees+slippage via costBps/stressMode; fail-closed, never forward-fill); beta-aware sizing (`estimateRollingBetas` strictly-before-window OLS, `scaleWeightsToTargetBeta` — targetBeta=0 via neutralization not ÷βp, fail-closed fallback; `inverseBetaTilt` in `beta-tilt.ts`); evaluation suite in `src/forest/alpha/cross-sectional-eval/` (Sharpe/Sortino annualized on portfolio return series, max drawdown on equity curve, long/short + cost attribution, gross/net exposure series, regime breakdown via injected labels only); wire-in seam `evaluateCrossSectional`. 90 new tests (2502 total), coverage 87.44% ≥ thresholds 82/85/85/82. Walk-forward composition deferred to Phase 6 | PR #4, commit `b7d5454` |
| **Alpha Research OS Phase 5** (2026-08-25) | Relative-value research: causal pair-spread engine `runPairSpreadSim` in `src/tree/alpha/relative-value/` (hedge ratio β(t) from data strictly < t via `estimateRollingHedgeRatio`; fail-closed tradability gate `validatePairTradable` — cointegration + finite half-life + correlation floor; 3-state entry-exit machine; one-sided turnover + cost stress modes); tree-local `src/tree/alpha/cost-stress.ts` layering fix (tree never imports forest); evaluation suite in `src/forest/alpha/relative-value-eval/` (Sharpe/Sortino, drawdown, cost attribution, realized-beta diagnostic, parallel `RelativeValueReport`); wire-in seam `evaluateRelativeValue`. 117 new tests (2619 total), coverage 88.03% ≥ thresholds 82/85/85/82. Multi-pair scan + walk-forward deferred to Phase 6 | PR #5, commit `b3f51fc` |
...
- **Tests:** 3,968 across 309 test files, full suite green (100% pass rate)
- **Coverage:** statements 91.63%, branches 90.99%, functions 95.06%, lines 91.63% (thresholds 90/90/94/90 global; per-path floors: exchange 100/100/100/100, bot 96/93/89/96, quantlib 100/100/100/100)
...
- **Production:** SHA `4989176b` (`4989176ba2dbe2be623fd417c263ff3ff25f2f9a`), health green (db/circuitBreaker/rateLimiter all `"ok"`, killswitch active)
...
- **Live derivative data** — documented status quo: all four `/fapi/v1/*` endpoints return HTTP 403 from this cloud environment; derivative fetchers are exercised with graceful degradation and deterministic offline injection.
