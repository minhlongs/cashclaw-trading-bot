...
| **Direct-D1 Read Store with TTL Cache** (2026-09-01) | Direct-D1 read architecture for BotManager (`getAllBots`, `getRunningBots`, `getBot`) eliminating in-memory state desynchronization across ephemeral Cloudflare Workers isolates. Cloudflare D1 is the single authoritative source of truth. Hot-path in-memory TTL cache (`CachedBot` with `BOT_CACHE_TTL_MS = 30_000`) prevents query exhaustion during high-frequency scheduler ticks and dashboard polling. Transparent D1 reads replace prior manual pre-hydration phases in scheduler and worker. Fallback configuration synthesis (`defaultConfigFromRow`) for corrupt JSON records. All quality gates green: 3729/3729 tests passing, typecheck 0, lint 0. | commits TBD |
