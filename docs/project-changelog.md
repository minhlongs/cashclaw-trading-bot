...
- **Research registry shipped** (`src/tree/alpha/registry/`): immutable, machine-readable registry of research entries with hypothesis, data sources, feature set, train/validation/OOS periods, costs, slippage, seed, git commit, result, falsification reason, and status (`PROPOSED | RUNNING | SURVIVED | FALSIFIED | ARCHIVED`). Duplicate hypothesis+config hashes rejected. Seeded with the 30 falsified classes from `docs/falsification-report.md` at class-level reproducibility, so re-testing a dead hypothesis is now machine-detectable. Pure functions, 25 tests.
- **Hypothesis lineage shipped** (`src/tree/alpha/hypothesis/lineage.ts` + `lineage-types.ts`): parent/mutation-tracked research graph with validated ID scheme (`H001 → H001-A → H001-A-REGIME`), cycle/self-parent rejection, ancestors/descendants, dead-end detection, and a `lineageToRegistryEntries()` bridge. Additive optional `parentId`/`mutation` on `AlphaHypothesis` — backward-compatible.
- **Microstructure feature contracts shipped** (`src/tree/alpha/microstructure/`): 9 causal features (bid-ask spread, order-book imbalance, depth imbalance, trade imbalance, aggressive volume, volume delta, liquidity shock, realized spread, price impact) declared through the existing `declareFeature()` gate — non-causal declarations throw. Sources `'orderbook'`/`'trades'`, availability `'when_listed'`; missing data stays `null` and is never forward-filled. Contracts only — no L2/L3 data fetching (that infrastructure is a later phase per the falsification report). 12 tests.
- **Cross-sectional universe shipped** (`src/tree/alpha/universe/`): `createUniverse()` (non-empty, unique symbols), `rankAssets()`, `percentileNormalize()`, `selectLongShort()`, `marketNeutralWeights()` (weights sum ≈ 0), `basketNeutralize()`. Pure and deterministic.
- **Regime transition matrix shipped** (`src/tree/regime/transition-matrix.ts`): 7×7 `P(regime[t+1] | regime[t])` built only from consecutive observed pairs (causal by construction), plus persistence probabilities, Shannon transition entropy, average duration per regime, hazard (1/avgDuration), and an `alphaDecayByRegime()` hook. Extends — does not replace — the classifier; consumes `RegimeHistoryStore` output directly. 19 tests, 97% covered.
- **Experiment metadata expanded** (`src/forest/alpha/experiments/`, additive only): `Experiment` gains optional `hypothesisId`/`parentId` (lineage links); `ExperimentResult` gains `falsificationReason`, `experimentHash` (SHA-256 over canonical JSON of config+seed+gitCommit via `lib/canonical-json.ts` — reproducibility), and `registryEntryId`. Hash determinism tested with and without `gitCommit`.
- **D1 migration + persistence** (`migrations/0009_research_registry.sql`): append-only `research_registry` and `research_hypotheses` tables, idempotent `CREATE TABLE IF NOT EXISTS`, applied locally via `npm run db:apply` (9/9). `PersistenceAdapter` extended with `saveRegistryEntry` / `listRegistry` / `saveHypothesisNode` / `loadLineage` on both D1 and JSON adapters; no mutation of historical rows anywhere.
- **Safety invariant:** research-side only — zero execution-path changes. Gates, killswitch, credentials, and promotion state machine untouched; no new order-placement path, no `LIVE` reachability change.
- **Quality gates:** 2129/2129 tests pass across 168 files (3 consecutive runs — flake check), coverage 85.82/86.98/92.35/85.82 (thresholds 82/85/85/82), lint 0 warnings, `tsc --noEmit` clean, all new files ≤200 lines, 0 `:any`, 0 new eslint-disable.
- **Docs:** `docs/alpha-research-os-implementation-plan.md` added (full-repo assessment + phased roadmap for the remaining mission phases); roadmap, changelog, and system architecture updated.
